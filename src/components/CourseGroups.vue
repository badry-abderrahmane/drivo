  ...
  <div class="course-groups">
    <v-expansion-panels v-model="open" multiple variant="popout" class="gap-4">
      ...
      >
        ...
        <!-- Lazy: this level's matière blocks mount only when the level opens. -->
        <v-expansion-panel-text class="pt-2 px-4 pb-4">
          <div
            v-for="sub in s.subjects"
            :key="sub.subject"
            data-test="subject-block"
            class="subject-block mb-4"
          >
            <!-- Matière heading: the level is divided by matière first, chapters below. -->
            <div class="d-flex align-center ga-2 mb-2 px-1">
              <v-icon icon="mdi-flask-outline" size="18" color="primary" />
              <span class="text-subtitle-2 font-weight-bold">{{ sub.subject }}</span>
